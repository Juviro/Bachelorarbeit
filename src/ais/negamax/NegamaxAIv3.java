...
public class NegamaxAIv3 {
    ...
    private final int AVERAGE_TURNS_PER_GAME = 100;
    ...
    public GameState performMove(GameState state) {
        ...
        // very low searching depth in the first 4 turns
        if (state.turn <= 8) {
            DEPTH = 3;
        //increase depth if 3 or less red bullets a re on the board
        } else if (state.capturedBulletsWhite + state.capturedBulletsBlack > 11) {
    ...
    private double negamax(GameState state, int depth, int color, double alpha, double beta) {
        ...
        // If the last move was a capture move, perform another iteration instead.
        if (depth <= 0 && !state.lastMove.isCaptureMove) {
